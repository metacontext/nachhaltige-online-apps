...
var svgRoot = document.getElementById("initiativen");
...
function mapApp(adjustVBonWindowResize,resizeCallbackFunction) {
	...
	if (!svgRoot.getScreenCTM) {
		...
		//this part is only required for viewers not supporting svgRoot.getScreenCTM() (e.g. ASV3)
...
//set viewBox of svgRoot to innerWidth and innerHeight
mapApp.prototype.adjustViewBox = function() {
	svgRoot.setAttributeNS(null,"viewBox","0 0 "+this.innerWidth+" "+this.innerHeight);
}
...
mapApp.prototype.calcInvCoord = function(svgPoint) {
	if (!svgRoot.getScreenCTM) {
		var matrix = getTransformToRootElement(svgRoot);
	}
	else {
...
mapApp.prototype.updateTooltip = function(evt) {
	...
	if (myText) {
		...
		for (var i=0;i<textArray.length;i++) {
			...
			var dy = this.tooltipTextAttribs["font-size"] + 4;
			if (i == 0) {
				var dy = 0;
			}
